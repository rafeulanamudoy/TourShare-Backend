...
const getSingleTeamByEmail = async (email) => {
  const result = await createTeam_model_1.CreateTeam.findOne({
    email: email,
  }).populate({
...
const updateSingleTeam = async (id, payload) => {
  const result = await createTeam_model_1.CreateTeam.findOneAndUpdate(
    { _id: id },
    payload,
    {
      new: true,
...
const acceptTeam = async (id, payload) => {
  const team = await createTeam_model_1.CreateTeam.findById(id).populate(
    "joinPeople"
  );
  if (!team) {
  ...
  const joinPerson = team.joinPeople.find(
    (person) => person.joinTeamId.toString() === payload.joinTeamId.toString()
  );
  ...
  if (
    ...
    throw new handleApiError_1.default(
      400,
      `Your Team Only Needed ${team.neededMembers}`
    );
  } else if (
    team.neededMembers === 0 &&
    payload.status === joinTeamStatus_1.ENUM_jOIN_TEAM_STATUS.ACCEPTED
  ...
  try {
    if (
      (joinPerson.status === joinTeamStatus_1.ENUM_jOIN_TEAM_STATUS.PENDING ||
        joinPerson.status ===
          joinTeamStatus_1.ENUM_jOIN_TEAM_STATUS.NOTACCEPTED) &&
      ...
      const result = await createTeam_model_1.CreateTeam.findOneAndUpdate(
        ...
        },
        {
          arrayFilters: [
            {
              "element.joinTeamId": new mongoose_1.default.Types.ObjectId(
                payload.joinTeamId
              ),
            },
          ],
      ...
      await joinTeam_model_1.JoinTeam.findByIdAndUpdate(
        new mongoose_1.default.Types.ObjectId(payload.joinTeamId),
      ...
      const result = await createTeam_model_1.CreateTeam.findOneAndUpdate(
        { _id: id },
        {
          $inc: {
            currentMembers:
              joinPerson.status ===
                joinTeamStatus_1.ENUM_jOIN_TEAM_STATUS.PENDING ||
              joinPerson.status ===
                joinTeamStatus_1.ENUM_jOIN_TEAM_STATUS.NOTACCEPTED
                ? payload.members
                : -payload.members,
...
const deleteSingleTeam = async (id) => {
  ...
  session.startTransaction();
  try {
    const deleteTeam = await createTeam_model_1.CreateTeam.findOneAndDelete(
      { _id: id },
      { session }
    ).populate({
      path: "joinPeople.joinTeamId",
    ...
    for (const joinPerson of joinPeople) {
      await joinTeam_model_1.JoinTeam.findByIdAndDelete(joinPerson.joinTeamId, {
        session,
      });
    }
    await session.commitTransaction();
